...
americaISOCurrentLeadPlot <- ggplot(site_proportions_top_10_now) +
...
americaISOOldLeadPlot <- ggplot(site_proportions_top_10_old) +
  geom_line(size=1,aes(x=yearCategory, y=proportion, color = 'grey',group=name)) +
  geom_point(aes(x=yearCategory, y=proportion,fill=name),shape=21, color="black", size=6) +
  scale_fill_manual(name = "Country", values = country_colors) +  # Apply consistent country colors for points
  scale_color_manual(name = "Country", values = country_colors) +
...
americaISOCurrentLeadPlot <- americaISOCurrentLeadPlot + theme(legend.position = "none")
americaISOOldLeadPlot <- americaISOOldLeadPlot + theme(legend.position = "none")
library(ggpubr)
americaTopResearchHosts2010sVs1960s <- ggarrange(americaISOCurrentLeadPlot, americaISOOldLeadPlot,
                                                nrow = 1, ncol = 2, labels = c("a", "b") )

americaTopResearchHosts2010sVs1960sLegend <- ggarrange(americaTopResearchHosts2010sVs1960s, country_legend, 
                                                      nrow = 2, ncol = 1, heights = c(2,1))
ggsave("americaStudySiteCurrentVsOldLead.png",plot = americaTopResearchHosts2010sVs1960sLegend, width = 10, height = 7, dpi = 600)


######################### america decadal map of all study sites #################

afr_data <- world_data %>% filter(continent == "South America" | continent == "North America")
...
ggsave("americaDecadalMap.png", plot = americaDecadalMap, width = 10, height = 8, dpi = 600)